...
var authorize = () => {
    return (socket, next) => {
        ...
        if (!query || typeof query.gid !== 'string') {
                ...
                Game
                    ...
                    .exec((err, game) => {
                        ...
                        var crossword = {
                                ...
                                whitesC: game.crossword.whitesC,
...
var connected = (socket) => {
    ...
    socket.on('letters to other', data => {
        ...
        if (!Array.isArray(letters) || !letters.length ||
            letters.length > game.crossword.whitesC) {
            return;
        }
        ...
        var len = letters.length,
            dateNow = Date.now(),
            checkLetter = letter => {
                if (!letter.date || letter.date > dateNow ||
                    typeof letter.letter !== 'string' ||
                    letter.letter.length !== 1 || !Array.isArray(letter.pos) ||
                    letter.pos.length !== 2
                ) {
                    return false;
                }
        ...
        (function uploadLetters(ind) {
            if (ind < len) {
                ...
                // Discard invalid letters and letters with date ahead of now
    ...
    socket.on('message to other', data => {
        if (typeof data !== 'string' || data.length < limits.MESSAGE_MIN_LENGTH ||
            data.length > limits.MESSAGE_MAX_LENGTH) {
            return;
        }
