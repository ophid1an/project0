$(function() {
    ...
    if (canvas.getContext) {
        ...
        // var gw = crossword.dimensions[1] * 50;
        // var gh = crossword.dimensions[0] * 50;
        ...
        // var cw = gw + (padding * 2) + 1;
        // var ch = gh + (padding * 2) + 1;
        ...
        console.log(cw + ' : ' + ch);
        var crossword = myCrossword1;
        var len = 20;
        var padding = 20;
        var ctx = canvas.getContext('2d');
        var letterModX = 3 * (len / 10);
        var letterModY = 7 * (len / 10);

        $(jcanvas).on('click', function() {
            drawMatrix(1.02);
        });

        $(jcanvas).on('dblclick', function() {
            drawMatrix();
        });
        ...
        $('#keyboard').keyboard({
            ...
            customLayout: {
                'normal': [
                    ...
                    '\u0395(E) \u03a1(R) \u03a4(T) \u03a5(Y) \u0398(U) \u0399(I) \u039f(O) \u03a0(P)',
    ...
    function drawMatrix(num) {
        ...
        num = num || 1;
        // var padding = padding / num;

        ctx.scale(num, num);

        for (var i = 0; i <= crossword.dimensions[0]; i++) {
            for (var j = 0; j <= crossword.dimensions[1]; j++) {
                if (i === 0) {
                    if (j !== 0) {
                        ctx.fillText(j, 0.5 + padding + len * j + letterModX, 0.5 + padding + len * i + letterModY);
                        // ctx.strokeRect(0.5 + padding + len * j, 0.5 + padding + len * i, len, len);
                    } else {
                        // console.log(0.5 + padding + len * j + letterModX + ' : ' + 0.5 + padding + len * i + letterModY);
                    }
                } else {
                    if (j === 0) {
                        ctx.fillText(i, 0.5 + padding + len * j + letterModX, 0.5 + padding + len * i + letterModY);
                        // ctx.strokeRect(0.5 + padding + len * j, 0.5 + padding + len * i, len, len);
                    } else {
                        ctx.strokeRect(0.5 + padding + len * j, 0.5 + padding + len * i, len, len);
                    }
        ...
        crossword.blackPositions.forEach(function(dim) {
            // console.log(dim[0] + ' : ' + dim[1]);
            ctx.fillRect(0.5 + padding + len * dim[1], 0.5 + padding + len * dim[0], len, len);
        });
